...
def signin(): 
    ...
    # define the data to replace the placeholders in the sql query
    data = (email, password)
    # execute the sql query using the cursor
    cursor.execute(sql, data)
    # we are using if else statements to check how many rows are returned
    count = cursor.rowcount
    # if condition to check if there are zero rows found then return no to user found
    if count == 0:
        return jsonify ({'message':'login failed'})
    else:
        # if there is a user in else we tell the cursor to pick only one row normally the first row
        user = cursor.fetchone()
        # return a response with the user information
        return jsonify ({'message':'login success', 'user': user})
    

 # defining the route of the api
...
# defining the corressponding function
def  get_product_details():
    ...
    # clossing the database connection
    connection.close()
    # returning a response to the user
